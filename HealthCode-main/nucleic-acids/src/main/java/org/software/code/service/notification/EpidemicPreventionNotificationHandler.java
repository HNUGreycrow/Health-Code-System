...
/**
 * EpidemicPreventionNotificationHandler 是一个用于处理疫情防控相关通知消息的处理器类。
 * 它实现了 NotificationHandler 接口，主要负责将特定类型的通知消息发送到 Kafka 主题。
...
public class EpidemicPreventionNotificationHandler implements NotificationHandler {
    ...
    /**
     * 注入 NotificationProducer 实例，用于将通知消息发送到 Kafka 主题。
     */
    ...
    /**
     * 处理通知消息的方法，将消息类型设置为 "EPIDEMIC"，并使用 NotificationProducer
    ...
    public void handle(NotificationMessageDto message) {
        // 将消息类型设置为 "EPIDEMIC"，表示这是一个疫情防控相关的通知消息
