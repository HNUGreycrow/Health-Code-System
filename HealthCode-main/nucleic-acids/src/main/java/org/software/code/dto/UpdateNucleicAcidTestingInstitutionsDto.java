...
/**
 * UpdateNucleicAcidTestingInstitutionsDto 是一个数据传输对象（DTO），
 * 用于封装更新核酸检测机构信息时所需的数据。
 * 借助 Lombok 的 @Data 注解自动生成 getter、setter、toString、equals 和 hashCode 等方法，
 * 通过 @JsonProperty 注解指定 JSON 序列化和反序列化时的字段名称，
 * 并使用 @NotNull 和 @NotBlank 注解对关键属性进行非空校验，确保数据的完整性和有效性。
 *
 * @author “101”计划《软件工程》实践教材案例团队
 */
@Data
public class UpdateNucleicAcidTestingInstitutionsDto {
    /**
     * 核酸检测机构的唯一标识 ID，用于确定要更新的具体机构。
     * 在 JSON 序列化和反序列化时对应 "id" 字段，该字段不能为空，否则会触发校验错误。
     */
